...
class ZmqProducer extends EventEmitter {
    ...
    async init(port) {
        ...
        const fullUrl = `tcp://127.0.0.1:${port}`;
    ...
    async send(...params) {
        ...
        socket.send(["observationMessages", JSON.stringify(params)]);
    }
}
...
const getAddressFromNetworkInterface = () => {
    ...
    if (results && results['Wi-Fi']) {
        return results['Wi-Fi'][0];
    }
    if (results && results['Ethernet 4']) {
        return results['Ethernet 4'][0];
